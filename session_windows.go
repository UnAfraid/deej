...
func (s *wcaSession) GetMute() bool {
	...
	if err := s.volume.GetMute(&muteState); err != nil {
...
func (s *wcaSession) SetMute(mute bool) error {
	...
	s.logger.Debugw("Adjusting session mute", "to", fmt.Sprintf("%v", mute))
...
func (s *masterSession) SetMute(v bool) error {
	...
	if err := s.volume.SetMute(v, s.eventCtx); err != nil {
		s.logger.Warnw("Failed to set session mute", "error", err, "mute", v)
		return fmt.Errorf("adjust session volume: %w", err)
	}

	s.logger.Debugw("Adjusting session mute", "to", fmt.Sprintf("%v", v))
